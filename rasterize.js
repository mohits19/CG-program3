...
var triangleTextures = []; // textures for each triangle
var ellipsoidTextures = []; //textures fro each ellipsoid model

var vertexBuffers = []; // this contains vertex coordinate lists by set, in triples
var normalBuffers = []; // this contains normal component lists by set, in triples
var triSetSizes = []; // this contains the size of each triangle set
var triangleBuffers = []; // lists of indices into vertexBuffers by set, in triples
var textureBuffers = [];  // this contains texture coordinates in doubles
...
var lightingModeULoc; // where to put lighting model for fragment shader
...
var lightingMode = 0; // which light texture blend mode to use
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "KeyB": // toggle between lighting modes
    ...
    function makeEllipsoid(currEllipsoid,numLongSteps) {
        ...
        try {
            if (numLongSteps % 2 != 0)
                ...
                var textureCoords = [0, 0];       // texels to return, init to (0, 0)
                ...
                for (var latAngle=-latLimitAngle; latAngle<=latLimitAngle; latAngle+=angleIncr) {
                    ...
                    for (var longAngle=0; longAngle<2*Math.PI; longAngle+=angleIncr){ // for each long
                        ...
                        textureCoords.push((longAngle)/(2 * Math.PI), (latLimitAngle - latAngle) / (2 * latLimitAngle));
                    }
            ...
            return({vertices:ellipsoidVertices, normals:ellipsoidNormals, triangles:ellipsoidTriangles, textures:textureCoords});
        } // end try
        ...
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glTexCoords.push(uvToAdd[0], 1 - uvToAdd[1]);          // put uv in list
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        uniform int uLightingMode; // the lighting mode
        ...
        uniform sampler2D uSampler; // the sampler
        ...
        void main(void) {
            ...
            vec4 textureOut = texture2D(uSampler, vTextureCoord);

            if(uLightingMode == 1)
                gl_FragColor = textureOut;
            else if(uLightingMode == 2)
                gl_FragColor = vec4(colorOut*textureOut.rgb, textureOut.a);
            else
                gl_FragColor = vec4(colorOut*textureOut.rgb, uAlpha*textureOut.a);
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                lightingModeULoc = gl.getUniformLocation(shaderProgram, "uLightingMode"); // ptr to lighting mode
...
function renderModels() {
    ...
    gl.uniform1i(lightingModeULoc, lightingMode); // pass in the lighting mode
    ...
    // render given trinagle set
    function renderTriangleSet(currSet){
    ...
    } // end render triangle set

    // render given ellipsoid
    function renderEllipsoid(ellipsoid){
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[numTriangleSets+ellipsoid.index]); // activate vertex buffer
        gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed vertex buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[numTriangleSets+ellipsoid.index]); // activate normal buffer
        gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[numTriangleSets+ellipsoid.index]); // activate tri buffer

        gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[numTriangleSets+ellipsoid.index]); // activate texture buffer
        gl.vertexAttribPointer(vTextureAttribLoc, 2, gl.FLOAT, false, 0, 0); // feed texture buffer to shader

        gl.bindTexture(gl.TEXTURE_2D, ellipsoidTextures[ellipsoid.index]);
        gl.activeTexture(gl.TEXTURE0);

        // draw a transformed instance of the ellipsoid
        gl.drawElements(gl.TRIANGLES,triSetSizes[numTriangleSets+ellipsoid.index],gl.UNSIGNED_SHORT,0); // render
    } // end render ellipsoid

    var currSet; // the tri set and its material properties
    var ellipsoid, instanceTransform = mat4.create(); // the current ellipsoid and material
    var transparentModels = []; // all transparent models
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        if(currSet.material.alpha == 1.0){        // if opaque, render
            renderTriangleSet(currSet);
        }
        else{                                     // if transparent, push to array to render later
            currSet.modelType = "triangle";
    ...
    for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
        ellipsoid = inputEllipsoids[whichEllipsoid];
        ellipsoid.index = whichEllipsoid;
        if(ellipsoid.alpha == 1.0){               // if opaque, render
            renderEllipsoid(ellipsoid);
        }
        else{                                     // if transparent, push to array to render later
            ellipsoid.modelType = "ellipsoid";
            transparentModels.push(ellipsoid);
        }
    }
    ...
    // sort array by z value
    transparentModels.sort(function(a, b){return b.center[2] - a.center[2]});

    // render models in order of depth
    for(var whichModel=0; whichModel < transparentModels.length; whichModel++){
        var currModel = transparentModels[whichModel];
        if(currModel.modelType == "triangle")
            renderTriangleSet(currModel);
        else
            renderEllipsoid(currModel);
    }

} // end render model

// setup textures for models
function setupTextures(){
  for(var i = 0; i < numTriangleSets; i++) {
      triangleTextures[i] = gl.createTexture();
      triangleTextures[i].image = new Image();
      triangleTextures[i].image.crossOrigin = "Anonymous";
      triangleTextures[i].image.src = "https://ncsucgclass.github.io/prog3/" + inputTriangles[i].material.texture;
      gl.bindTexture(gl.TEXTURE_2D, triangleTextures[i]);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, triangleTextures[i].image);
      gl.bindTexture(gl.TEXTURE_2D, null);
  }

  for (var i = 0; i < numEllipsoids; i++) {
      ellipsoidTextures[i] = gl.createTexture();
      ellipsoidTextures[i].image = new Image();
      ellipsoidTextures[i].image.crossOrigin = "Anonymous";
      ellipsoidTextures[i].image.src = "https://ncsucgclass.github.io/prog3/" + inputEllipsoids[i].texture;
      gl.bindTexture(gl.TEXTURE_2D, ellipsoidTextures[i]);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, ellipsoidTextures[i].image);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
      gl.bindTexture(gl.TEXTURE_2D, null);
...
function main() {
  ...
  setupTextures(); // setup textures for all models
